...
class RemoteFeedLoaderTests: XCTestCase {
    ...
    func test_load_on200HTTPResponseWithEmptyJSONList_deliversNoItems() {
        let (sut, client) = makeSut()
    ...
    func test_load_on200HTTPResponseWithJSONItems_deliversItems() {
        let (sut, client) = makeSut()
        let item1 = makeItem(
            id: UUID(),
    ...
    func test_load_doesNotDeliverResultAfterSUTHasBeenDealloated() {
    ...
    private func expect(_ sut: RemoteFeedLoader, toCompleteWith expectedResult: RemoteFeedLoader.Result, when action: () -> Void, file: StaticString = #filePath, line: UInt = #line) {
        ...
        sut.load { receivedResult in
            ...
            default:
                XCTFail("Expected result \(expectedResult), but received \(receivedResult) instead", file: file, line: line)
            }
            expectation.fulfill()
        }
